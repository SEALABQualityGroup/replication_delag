...
DEP_DATE = str(date.today() + timedelta(days=1)) #"2021-01-08"
...
class Requests:
    ...
    def search_ticket(self, departure_date, from_station, to_station):
        if random() > 0.7:
            self.search_ticket_norm(departure_date, from_station, to_station)
        else:
            self.search_ticket_adv(departure_date, from_station, to_station)


    def search_ticket_norm(self, departure_date, from_station, to_station):
    ...
    def search_ticket_adv(self, departure_date, from_station, to_station):
        head = {"Accept": "application/json", "Content-Type": "application/json"}
        body_start = {"startingPlace": from_station, "endPlace": to_station, "departureTime": departure_date}

        req_label = sys._getframe().f_code.co_name
        self.client.post(url="/api/v1/travelplanservice/travelPlan/cheapest", headers=head, json=body_start, catch_response=True, name=req_label)
